...
export default {
  ...
  methods: {
    async pujar() {
      ...
      await axios
        .get(`http://167.99.240.123:3001/miDinero/${cookie.id_cliente}`)
        .then(async (res) => {
          ...
          if (res.data.saldo.saldo < parseInt(importe)) {
            ...
            await axios
            .post(
              `http://167.99.240.123:3001/subasta/${this.$route.params.id}/${this.$route.params.categoria}/${this.$route.params.titulo}`,
    ...
    async addFavourites(id_producto, created_by) {
      ...
      await axios.post(`http://167.99.240.123:3001/favourites`, object);
      await axios.post(`http://167.99.240.123:3001/favouritesProducto`, object);
    },
    async delFavourites(id_producto, created_by) {
      ...
      object["cookie"] = this.$cookies.get("loginCookie").id_cliente;
      console.log(object);
      await axios.post(`http://167.99.240.123:3001/favouritesDel`, object);
      await axios.post(`http://167.99.240.123:3001/favouritesProductoDel`, object);
    },
  },
  created() {
    setInterval(() => {
  ...
  async mounted() {
    this.socket = io("http://167.99.240.123:3001");
    ...
    await axios
      .get(`http://167.99.240.123:3001/subasta/${this.$route.params.id}`)
      .then((res) => {
        console.log(this.$route.params.id);
        const { data } = res;
        this.arrayImg = data;
    ...
    await axios
      .get(`http://167.99.240.123:3001/perfil/${this.$route.params.id}`)
      .then((res) => {
        const { data } = res;
